...
class EmprestimosView(BaseModuleView):
    ...
    def __init__(self, master, theme, screen_manager):
        ...
        super().__init__(master, theme, screen_manager, "emprestimos")

        # Corrigido: Removido o 'return' que quebrava o fluxo inicializador
        ...
        #
        FormLabel(
            self, text="Livro:", x=FORM_LABEL_X, y=FORM_BOOK_Y, width=FORM_LABEL_WIDTH
        )
        FormLabel(
            self,
            text="Utilizador:",
        ...
        self.combo_livro = FormComboBox(
            self, values=[], x=FORM_FIELD_X, y=FORM_BOOK_Y, width=FORM_FIELD_WIDTH
        )
        self.combo_utilizador = FormComboBox(
            self, values=[], x=FORM_FIELD_X, y=FORM_USER_Y, width=FORM_FIELD_WIDTH
        )
        self.entry_data = FormEntry(
            self, x=FORM_FIELD_X, y=FORM_DATE_Y, width=FORM_FIELD_WIDTH
        )
        self.entry_prevista = FormEntry(
            self, x=FORM_FIELD_X, y=FORM_RETURN_Y, width=FORM_FIELD_WIDTH
        )
        ...
        hoje = datetime.today()
        self.entry_data.set_value(hoje.strftime("%d/%m/%Y"))
        self.entry_prevista.set_value((hoje + timedelta(days=7)).strftime("%d/%m/%Y"))
    ...
    def carregar_livros(self):
        ...
        self.combo_livro.set_values(
            [livro.titulo for livro in self.livros if livro.disponivel]
        )

    def carregar_utilizadores(self):
        self.utilizadores = self.utilizador_service.listar()
        self.combo_utilizador.set_values(
            [utilizador.nome for utilizador in self.utilizadores]
        )
